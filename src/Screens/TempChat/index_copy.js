...
const TempChat = ({ navigation, route }) => {
    ...
    useEffect(() => {
        ...
        socket.current.emit('temporary_chat_between_two_users_agian', {
            room_id: room_id,

        });

        socket.current.on('temporary_chat_between_two_users_agian', (data) => {
    ...
    useEffect(() => {
        socket.current.on('temporary_single_data', (data) => {
    ...
    const sendMessage = () => {
        if (inputText == '') {
            ...
            socket.current.emit("temporary_chat", data)
            setInputText('');
        }
    };
